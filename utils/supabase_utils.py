...
def get_labeled_clips():
    response = supabase.table("your_table_name").select("filename", "label").execute()
    return response.data

def download_wav(filename, dest_folder="clips"):
    os.makedirs(dest_folder, exist_ok=True)
    url = f"{SUPABASE_URL}/storage/v1/object/public/nfc-uploads/{filename}"
